...
class SessionManager:
    ...
    def list_sessions(self, limit: int = 10):
        with sqlite3.connect(self.db_path) as conn:
            ...
            # Instead of fetching parent status inside the loop, we fetch all
            # required parents in a single batch query to avoid multiple round trips.
            parent_ids = {s['parent_id'] for s in sessions if s['status'] == 'running' and s['parent_id']}
            parents = {}
            if parent_ids:
                # The '?' placeholder syntax requires a sequence, so we convert the set to a list or tuple.
            ...
            # Group all status updates into a single transaction.
            if sessions_to_update:
                conn.executemany("UPDATE sessions SET status = 'crashed' WHERE id = ?", [(sid,) for sid in sessions_to_update])
                conn.commit()
