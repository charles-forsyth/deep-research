...
class SessionManager:
    ...
    def list_sessions(self, limit: int = 10):
        with sqlite3.connect(self.db_path) as conn:
            ...
            running_sessions = [s for s in result if s['status'] == 'running']

            if not running_sessions:
                return result

            # --- BATCH OPTIMIZATION ---
            # 1. Batch-fetch parent data to solve N+1 query problem
            parent_ids_to_fetch = {s['parent_id'] for s in running_sessions if s['parent_id']}
            parents = {}
            if parent_ids_to_fetch:
                # Use a placeholder string that can be safely formatted
            ...
            # 2. Identify all dead sessions in one pass
            crashed_session_ids = []
            for s in running_sessions:
                ...
                if s['pid']:
                    try:
                        # Signal 0 is a no-op, just checks for process existence
                        os.kill(s['pid'], 0)
                    except OSError:
                        is_dead = True
                # If no PID, check parent status from our batch-fetched data
                ...
                if is_dead:
                    ...
                    s['status'] = 'crashed' # Update in-memory dict

            # 3. Batch-update crashed sessions in a single query
