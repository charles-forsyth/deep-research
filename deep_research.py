...
class SessionManager:
    ...
    def list_sessions(self, limit: int = 10):
        with sqlite3.connect(self.db_path) as conn:
            ...
            # Pre-fetch all relevant parent sessions in a single query to avoid
            # hitting the database inside the loop for each child session.
            ...
            if sessions_to_update:
                conn.execute(
                    f"UPDATE sessions SET status = 'crashed' WHERE id IN ({','.join('?' for _ in sessions_to_update)})",
                    sessions_to_update
                )
                conn.commit()
