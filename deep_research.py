...
class SessionManager:
    ...
    def append_to_result(self, interaction_id: str, new_content: str):
        with sqlite3.connect(self.db_path) as conn:
            # Optimization: Use a single UPDATE with concatenation to avoid a round-trip
            # We use COALESCE to handle NULL results and || for concatenation
            conn.execute(
    ...
    def list_sessions(self, limit: int = 10):
        with sqlite3.connect(self.db_path) as conn:
            conn.row_factory = sqlite3.Row
            # Use LEFT JOIN to fetch parent info in one go, avoiding N+1 queries
            query = """
            ...
            for s in sessions:
                ...
                if s["status"] == "running":
                    ...
                    # 2. Check Parent Status/PID (using joined columns)
                    elif s["parent_id"]:
                        if s["parent_status"]:
                            # If parent is finished, child should be finished.
                            if s["parent_status"] in [
                                "completed",
                                "crashed",
                                "failed",
                                "cancelled",
            ...
            # Batch update crashed sessions to reduce DB round-trips
            if crashed_ids:
...
Set GEMINI_API_KEY in a local .env file or at ~/.config/deepresearch/.env
    ...
    parser_research.add_argument(
        ...
        "--quiet",
        action="store_true",
        help="Suppress logs, output only final report (Good for piping)",
    )
    parser_research.add_argument(
        "--stream",
        action="store_true",
        help="Stream the agent's thought process (Recommended)",
    )
    parser_research.add_argument(
        "--stores", nargs="+", help="Existing Cloud File Search Store names (advanced)"
    )
    parser_research.add_argument(
        "--upload",
        nargs="+",
        help="Local file/folder paths to upload, analyze, and auto-delete",
    )
    parser_research.add_argument(
        "--format",
        help="Specific output instructions (e.g., 'Technical Report', 'CSV')",
    )
    parser_research.add_argument(
        "--output", help="Save report to file (e.g., report.md, data.json)"
    )
    parser_research.add_argument(
        "--depth", type=int, default=1, help="Recursive research depth (default: 1)"
    )
    parser_research.add_argument(
        "--breadth",
        type=int,
        default=3,
        help="Max child tasks per recursion level (default: 3)",
    )
    parser_research.add_argument("--adopt-session", type=int, help=argparse.SUPPRESS)

    # Command: start (Headless)
    parser_start = subparsers.add_parser(
        "start", help="Start a research task in the background (Headless)"
    )
    ...
    parser_followup = subparsers.add_parser(
        "followup", help="Ask a follow-up question to a previous session"
    )
    ...
    parser_show.add_argument(
        "--recursive",
        action="store_true",
        help="Include all child session reports in output",
    )
    ...
    parser_estimate.add_argument(
        "--upload", nargs="+", help="Files to upload (adds to context cost)"
    )
    ...
    try:
        if args.command == "start":
            ...
            print(
                f"[INFO] Research started in background! (Session ID: {sid}, PID: {pid})"
            )
            ...
            if args.recursive:
                ...
                if not full_content:
                    ...
                    if args.save:
                        ...
                        console.print(
                            f"[bold green]Recursive report saved to {args.save}[/]"
                        )
                return
            ...
            if not session:
                show_console.print(
                    f"[bold red][ERROR] Session '{args.id}' not found.[/]"
                )
            else:
                show_console.print(
                    Panel(
            ...
            if not args.force:
                confirm = input(
                    f"Are you sure you want to delete {len(stores)} stores? [y/N] "
                )
                if confirm.lower() != "y":
                ...
                for s in stores:
                    ...
                    try:
                        if hasattr(client.file_search_stores, "documents"):
                            docs = list(
                                client.file_search_stores.documents.list(parent=s.name)
                            )
                            if docs:
            ...
            def build_tree(node_id, tree_node):
                ...
                for child in children:
                    status_style = (
                        ...
                        if child["status"] == "crashed" or child["status"] == "failed"
                        else "yellow"
                    )
            ...
            if args.id:
                ...
                for r in roots:
                    status_style = (
                        ...
                        if r["status"] == "crashed" or r["status"] == "failed"
                        else "yellow"
                    )
            ...
            if args.action == "login":
                ...
                console.print(
                    f"[bold green]Success![/] Key saved to {user_config_path}"
                )
            ...
            AVG_INPUT_TOKENS = (
                60_000  # Search results + web pages + internal thought trace
            )
