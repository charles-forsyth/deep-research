...
class SessionManager:
    ...
    def list_sessions(self, limit: int = 10):
        with sqlite3.connect(self.db_path) as conn:
            ...
            # required parents in a single query and look them up in a hash map.
            # This reduces DB queries from O(N) to O(1) in the worst case.

            running_sessions = [s for s in sessions if s['status'] == 'running']
            parent_ids = {s['parent_id'] for s in running_sessions if s['parent_id']}

            parents = {}
            if parent_ids:
                # Use a placeholder for a single-item tuple to avoid syntax errors
            ...
            for s in sessions:
                ...
                if s['status'] == 'running':
                    ...
                    if s['pid']:
                        try:
                            # A signal of 0 tests if the process exists
                            os.kill(s['pid'], 0)
                        except OSError:
                            is_dead = True
            ...
            # Update all crashed sessions in a single transaction instead of one by one.
            if crashed_ids:
